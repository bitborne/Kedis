...
- [Kedis: 高性能 Linux 内核增强型存储系统](#kedis-高性能-linux-内核增强型存储系统)
  ...
  - [快速开始](#快速开始)
    ...
    - [运行指南](#运行指南)
      ...
      - [启动从节点（应用层复制）](#启动从节点应用层复制)
  - [性能测试](#性能测试)
    - [Hardware Spec](#hardware-spec)
    - [通用测试参数](#通用测试参数)
    - [KSF 加载时间](#ksf-加载时间)
    - [Pipeline 性能：Redis VS Kedis](#pipeline-性能redis-vs-kedis)
    - [AOF 性能（预热后）：Redis VS Kedis](#aof-性能预热后redis-vs-kedis)
    - [增量同步性能](#增量同步性能)
    - [RDMA vs sendfile](#rdma-vs-sendfile)
      - [RDMA send（soft-RoCE）](#rdma-sendsoft-roce)
      - [sendfile](#sendfile)
    - [内存分配器对比：kmem vs jemalloc](#内存分配器对比kmem-vs-jemalloc)
      - [1. jemalloc — pidstat 内存趋势](#1-jemalloc--pidstat-内存趋势)
      - [2. kmem — pidstat 内存趋势](#2-kmem--pidstat-内存趋势)
      - [3. QPS 数据对比](#3-qps-数据对比)
  - [发展路线](#发展路线)
  - [贡献](#贡献)
  - [支持](#支持)
  - [许可证](#许可证)
