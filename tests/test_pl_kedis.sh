...
OUTPUT_DIR="results_kedis"
mkdir -p "$OUTPUT_DIR"

echo "========== Kedis (HSET/HGET/HDEL) Pipeline 压测 | PORT=$PORT =========="
...
for PL in "${PIPELINES[@]}"; do
    ...
    run_test $PL "HSET" "HSET __key__ __data__"
    run_test $PL "HGET" "HGET __key__"
    run_test $PL "HDEL" "HDEL __key__"
    if [ "$PL" != "$LAST_PL" ]; then
        echo ""
        echo "========================================"
        echo "Pipeline=$PL 测试完成"
        echo "请重启你的 Kedis 服务，确认就绪后按 Enter 继续"
        echo "========================================"
        read -p ""
    fi
done
